...
- **Single rollback implementation** — Settle is the only component that handles rollbacks. Downstream targets receive clean changes (inserts, updates, deletes) and never need to reason about chain forks.
...
- **Minimal downstream writes** — Settle computes diffs and emits only changed rows, reducing write amplification on the target database.
- **Backpressure with eager merging** — When the downstream target is slow, Settle continues to accept and merge incoming batches, reducing the total number of flushes needed.
...
Settle is an **embedded library** — a Rust engine with host language bindings (napi-rs for Node.js, PyO3 for Python, etc.). It runs in the same process as the host application. No separate service to deploy.
...
Settle exposes itself as a target that wraps a single downstream target. Example in TypeScript (Pipes SDK):
...
source.pipe(decoder).pipeTo(settle)
```
...
1. Settle accepts a batch from the source and immediately updates its internal raw tables, reducers, and MVs.
2. It then attempts to flush the computed changes to the downstream target.
3. If the downstream has not acknowledged the previous flush, Settle continues accepting and merging incoming batches internally.
4. When the downstream acks, Settle flushes the accumulated change (which may now represent multiple merged batches — reducing write amplification).
5. The source is only backpressured if Settle's internal buffer exceeds a configurable memory limit.
...
Settle uses an embedded key-value store (RocksDB or redb) for its internal state:
...
Settle supports a configurable maximum unfinalized window. If exceeded, a warning is logged but processing continues.

### 7.4 Crash recovery

Settle uses a write-ahead log (WAL) for atomicity:
...
2. Host SDK calls settle.fork(previousBlocks)
3. Settle resolves fork cursor via resolveForkCursor() algorithm
4. Settle rolls back internal state:
...
5. Settle computes compensating change records
6. Settle flushes compensating changes to downstream targets
7. Settle returns fork cursor to host SDK
...
1. Settle removes the per-block contributions for block 1003 from each accumulator
...
const settle = new Settle({
  storage: './data/settle',
  schema: './schema.sql',
  target: new ClickHouseSettleTarget({ url: 'http://localhost:8123', database: 'dex' }),
})

await source.pipe(decodeSwaps).pipeTo(settle)
```
...
settle = Settle(
    storage="./data/settle",
    schema="./schema.sql",
    target=ClickHouseSettleTarget(url="http://localhost:8123", database="dex"),
)
await source.pipe(decode_swaps).pipe_to(settle)
```

### What ClickHouse receives

Settle auto-creates these tables in ClickHouse and streams changes into them:

```sql
-- Raw swaps (append-only from Settle's perspective)
CREATE TABLE swaps (...) ENGINE = MergeTree() ORDER BY (block_number);

-- PnL per 5m window (upserted by Settle on each flush)
...
One Settle instance per chain. Multi-chain coordination (e.g., cross-chain aggregation) is handled at a higher level by running multiple pipelines and merging in the downstream target.
...
When downstream is slow, Settle merges pending batches:
...
- Distributed Settle (sharded by group key)
- Temporal joins
...
| Feature | Settle | Materialize | ClickHouse MVs | Flink | Pipes SDK Aggregator |
|---------|----------|-------------|----------------|-------|---------------------|
...
**Philosophy:** Maximum performance and language freedom. Users write reducers in any language that compiles to WebAssembly (Rust, Go, AssemblyScript, C, etc.), and Settle executes the compiled module.
...
import { State, Row, Emit } from "@subsquid/settle-as";
...
- SDK: provide `settle_sdk` crate for Rust, `@subsquid/settle-as` for AssemblyScript
