...
pub struct LuaRuntime {
    ...
    /// Opt-out of Sync: the Lua VM must not be accessed concurrently.
    ...
    fn process(&self, state: &mut HashMap<String, Value>, row: &Row) -> crate::error::Result<Vec<RowMap>> {
        ...
        if self.state_fields.is_empty() {
            ...
            let (multi_count, emits_array, emit_table): (i64, mlua::Table, mlua::Table) =
                call.call(()).map_err(|e| {
                    crate::error::Error::Reducer(format!("Lua script execution failed: {e}"))
                })?;
            ...
            Ok(collect_emits(multi_count, &emits_array, &emit_table))
        } else {
            // Specialized path: state passed as positional args, returned as extra values.
            ...
            let result: MultiValue = call.call(args).map_err(|e| {
                crate::error::Error::Reducer(format!("Lua script execution failed: {e}"))
            })?;
            ...
            let emits_array = match iter.next() {
                ...
                _ => return Err(crate::error::Error::Reducer(
